 ...
 * @author sofia
 */
@WebServlet("/Admin/Instituciones/editar")
public class EditarInstitucionServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EditarInstitucionServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet EditarInstitucionServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            Institucion inst = institucionDB.encontrarPorId(id);
            if (inst == null) {
                String msg = URLEncoder.encode("Institución no encontrada", StandardCharsets.UTF_8.toString());
                resp.sendRedirect(req.getContextPath() + "/Admin/Instituciones/listar?error=" + msg);
                return;
            }
            req.setAttribute("institucion", inst);
            req.getRequestDispatcher("/Admin/Instituciones/editarInstitucion.jsp").forward(req, resp);
        } catch (Exception e) {
            String msg = URLEncoder.encode("Error: " + e.getMessage(), StandardCharsets.UTF_8.toString());
            resp.sendRedirect(req.getContextPath() + "/Admin/Instituciones/listar?error=" + msg);
        }
    }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            if (nombre == null || nombre.trim().isEmpty()) {
                ...
                resp.sendRedirect(req.getContextPath() + "/Admin/Instituciones/editar?id=" + id + "&error=" + msg);
                return;
            }

            institucionDB.actualizar(id, nombre.trim());
            String ok = URLEncoder.encode("Institución actualizada", StandardCharsets.UTF_8.toString());
            resp.sendRedirect(req.getContextPath() + "/Admin/Instituciones/listar?ok=" + ok);
        } catch (SQLException e) {
            String msg = URLEncoder.encode("Error al actualizar: " + e.getMessage(), StandardCharsets.UTF_8.toString());
            resp.sendRedirect(req.getContextPath() + "/Admin/Instituciones/listar?error=" + msg);
        }
    }
