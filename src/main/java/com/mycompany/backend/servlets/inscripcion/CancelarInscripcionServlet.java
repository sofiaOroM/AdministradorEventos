...
public class CancelarInscripcionServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet CancelarInscripcionServlet</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet CancelarInscripcionServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            inscripcionDB.eliminar(usuarioId, actividadId);
            resp.sendRedirect(req.getContextPath() + "/Actividades/listar?ok=Inscripción+cancelada");

        } catch (SQLException e) {
            req.setAttribute("error", "Error al cancelar inscripción: " + e.getMessage());
