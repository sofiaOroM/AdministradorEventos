 ...
 * @author sofia
 */
@WebServlet("/Actividades/eliminar")
public class EliminarActividadServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EliminarActividadServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet EliminarActividadServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            actividadDB.delete(id);
            resp.sendRedirect(req.getContextPath() + "/Actividades/listar?congresoId=" + congresoId);
        } catch (SQLException e) {
            req.setAttribute("error", "Error al eliminar actividad: " + e.getMessage());
